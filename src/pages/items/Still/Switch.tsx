import { Agrega, Edita, Detalles, Busca } from ".";
  ...
  const selector = () => {
    switch(action){
      case 'menu':
        ...
        break;
      case 'agrega':
        setCurrentPermissionNeeded([Permisos.STILLS_WRITE]);
        setElementToShow(<Agrega/>);
        break;  
      case 'busca':
        setCurrentPermissionNeeded([Permisos.STILLS_READ]);
      ...
      default: 
        setCurrentPermissionNeeded([]);
        setElementToShow(<NotFound errorMessage={`No existe la acción en Stills: ${action}`}/>);
        break;
    }
  }
