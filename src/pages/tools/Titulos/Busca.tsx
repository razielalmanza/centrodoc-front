...
export const Busca = (props: any) => {
  ...
  useEffect(() => {
    document.title = 'Busca Títulos'; // refactor con los dons id del url}
    if(query){
    //   onSubmit({titulo_original: query, titulo_en_espa: query});
      addToast(`Se ha usado la busq. rápida: [${query}] para completar los campos de búsqueda.`, {appearance: 'info'});
    }
  }, []);

  useEffect(() => {
    /** If necesario, pues en la montura de lapágina la búsqueda es vacia.
     * TIene contenido hasta que pasa por el onSubmit del componente avanzado.
     */
    if(busqueda)
      search(busqueda);
   }, [page]);
  ...
  const AvanzadaComponente = (props: any) => {
    const { onSubmit, defaultSearch } = props;
    const { register, handleSubmit } = useForm<Titulo>();
  ...
  return (
    <Container component="main" maxWidth="xl">
      <AvanzadaComponente onSubmit={onSubmit} defaultSearch={query}/>
      {resultadosBusqueda && 
